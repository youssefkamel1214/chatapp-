...
class Messegbuble extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    double width = min(140.0, 12.sp * max(messege.length, username.length));
        ...
        Row(
          ...
          children: [
            Container(
              ...
              width: type == 'image' || type == 'sound' || type == 'video'
                  ? null
                  : width,
              child: Column(
                crossAxisAlignment:
                ...
                children: [
                  ...
                  if (type == 'image') make_image(context),
                  if (type == 'sound')
                    make_sound(
                      url: messege,
                      duration: duration!,
                  ...
                  if (type == 'location') makelocation(messege),
                  if (type == 'doc') make_doc(context, messege, exte!),
                  if (isMe && type != 'video')
                    SizedBox(
                      width: type == 'text' || type == 'location'
                          ? width
                          : type == 'image'
                              ? 150
                              : type == 'sound'
  ...
  make_image(BuildContext context) {
    return GestureDetector(
        child: Hero(
          tag: key.toString(),
          child: Container(
            child: messege.contains('wiaitng')
                ? const Center(
                    child: CircularProgressIndicator(),
                  )
                : null,
            ...
            height: 35.w,
            decoration: BoxDecoration(
                image: messege.contains('wiaitng')
                    ? null
                    : DecorationImage(
                        fit: BoxFit.fill,
                        image: Image.network(
        ...
        onTap: imageurl.contains('wiaitng')
            ? null
    ...
    return Center(
      child: IconButton(
          onPressed: () async {
            ...
            if (dic == null) throw 'un acceptable';
...
class _make_soundState extends State<make_sound> {
  Duration max_dur = const Duration(microseconds: 1),
  ...
  Widget build(BuildContext context) {
    ...
    if (ind.inMilliseconds > 0) todis = '${ind.inMinutes}:${ind.inSeconds}';
    return Row(
      children: [
        if (still_loading)
        ...
        if (!still_loading)
          IconButton(
              onPressed: () => !playing ? start_play() : pause_play(),
        ...
        Container(
          width: 35.w,
          child: Slider(
            value: ind.inMilliseconds.toDouble(),
            max: max_dur.inMilliseconds.toDouble(),
            onChanged: (val) => audiomanger.audioPlayer
  ...
  Future<void> inlaze_var() async {
    try {
      ...
      listhener = audiomanger.audioPlayer.positionStream.listen((event) {
        if (audiomanger.URL != url2) cancel_stream();
        setState(() {
          ind = event;
  ...
  @override
  void dispose() {
    if (listhener != null) listhener!.cancel();
    playing = false;
    if (audiomanger.URL == widget.url) {
      audiomanger.audioPlayer.stop();
      audiomanger.make_empty();
    }
    if (_core != null && still_loading) _core!.cancel();
    super.dispose();
  }

  start_play() async {
    if (audiomanger.URL != url2) await inlaze_var();
  ...
  Future<void> dowlnload_file() async {
    _core =
        await dowlouadfromurl('${widget.id}.aac', 'records', widget.url, (val) {
      setState(() {
        _percent = val;
      });
    }, null);
...
class _make_videoState extends State<make_video> {
  ...
  void dispose() {
    ...
    if (_core != null && _controller == null) _core!.cancel();
  }

  @override
  Widget build(BuildContext context) {
    _controller == null ? null : print(_controller!.value.size);
    return Container(
      width: 220,
  ...
  Future<void> dowlnload_file(String filepath) async {
    _core =
        await dowlouadfromurl('${widget.id}.mp4', 'videos', widget.url, (val) {
      setState(() {
        _percent = val;
      });
    }, () {
      _controller =
          VideoPlayerController.file(File(filepath + '/${widget.id}.mp4'));
...
Future<DownloaderCore> dowlouadfromurl(String filename, String dicname,
    String url, Function? fnc, Function? fnc4) async {
  Directory? dic = await getExternalStorageDirectory();
  if (dic == null) throw 'un acceplte';
