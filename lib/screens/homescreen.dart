...
class MyHomePage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: Obx(() => chatchannelcont.issearchmode.value
          ? getsearchscreen()
          : show_chat_channels()),
    );
  }
  ...
  AppBar makeappbar(BuildContext context) {
    return AppBar(
      centerTitle: true,
      leading: IconButton(
        icon: Obx(() => chatchannelcont.issearchmode.value
            ? const Icon(Icons.arrow_back)
            : const Icon(Icons.search)),
        onPressed: () => chatchannelcont.siwitch_state_of_search(),
      ),
      title: Obx(() => chatchannelcont.issearchmode.value
          ? TextField(
        ...
        builder: (context, snapshot) {
          return Obx(() {
            ...
            if (chatchannelcont.channels.isEmpty || !snapshot.hasData)
              return const Center(
                child: Text('sorry you havent add anyone in channel '),
              );
            dynamic allchannels = snapshot.data;
            allchannels = allchannels.docs;
            if (allchannels.length > 0)
              allchannels.sort((channel1, channel2) {
                if (channel1['lasttime']
                    .toDate()
                    .isBefore(channel2['lasttime'].toDate()))
                  return 1;
                else if (channel2['lasttime']
                    .toDate()
                    .isBefore(channel1['lasttime'].toDate()))
                  return -1;
                else
                  return 0;
              });
            ...
            return ListView.builder(
              ...
              itemBuilder: (ctx, index) {
                int currnindex = chatchannelcont.channels.indexWhere(
                        (element) => element == allchannels[index].id),
                    differindex = allchannels[index]['users']
                        .indexWhere((elment) => elment == email);
                differindex = 1 - differindex;
                Duration diff = DateTime.now()
                    .difference(allchannels[index]['lasttime'].toDate());
                final imagesurls = (allchannels[index]['usersimages'] as List)
                    .map((e) => e as String)
                    .toList();
                final useremails = (allchannels[index]['users'] as List)
                    .map((e) => e as String)
                    .toList();
                return ChannelShower(
                    title: chatchannelcont.usernames[currnindex],
                    imageurl: allchannels[index]['usersimages'][differindex],
                    lastmessege: allchannels[index]['lastmessege'],
                    type: allchannels[index]['type'],
                    time: diff.inDays > 2
                        ? '${dmy.format(allchannels[index]['lasttime'].toDate())} '
                        : diff.inDays > 0
                            ? 'Yesterday'
                            : hm.format(
                                allchannels[index]['lasttime'].toDate()),
                    function: () =>
        ...
        return Obx(() {
          List<dynamic> users = docs.where((element) {
            ...
            return (s.contains(chatchannelcont.searchemail.value) ||
                    element['username']
                        .contains(chatchannelcont.searchemail.value) ||
                    chatchannelcont.searchemail.value.isEmpty) &&
                s != email;
          }).toList();
          if (users.isEmpty)
            return const Center(
              child: Text("we have found nothing like this"),
            );
          return ListView.builder(
              itemCount: users.length,
              itemBuilder: (contxt, index) {
                int currindex = chatchannelcont.users
                    .indexWhere((el) => el == users[index].id);
                return SearchUser(
                    imageurl: users[index]['image_url'],
                    useremail: users[index].id,
                    username: users[index]['username'],
                    isexist: currindex != -1,
                    function1: () => chatchannelcont.addtochannel(users[index]),
                    function2: () =>
