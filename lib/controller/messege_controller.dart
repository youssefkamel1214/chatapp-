...
class Messegcontroller extends GetxController {
  ...
  void inilize_messege_steam() {
    final s2 = FirebaseFirestore.instance
        .collection('chat_channel/$id/messages')
    ...
    streamSubscription2 = s2.listen((event) async {
      ...
      messeges.insertAll(
          0, data.where((element) => !messeges.contains(element)));
      ...
      if (inialindex == -1 && messeges.isNotEmpty) {
        inialindex = messeges.indexWhere((elment) {
          return (!(elment['seen'].contains(false))) ||
              elment['email'] == useremail;
        });
        if (inialindex == -1) inialindex = messeges.length;
      }
  ...
  void updateseen(int index) {
    if (messeges[index]['email'] == useremail) return;
  ...
  Future<void> update_seen(dynamic data) async {
    do {
      int tmp = -1;
      int id = messeges.indexWhere(
        (element) {
          tmp = data.indexWhere((element2) => element.id == element2);
          if (tmp == -1) return false;
          if (element['seen'] != data[tmp]['seen']) return true;
          return false;
        },
      );
      if (id != -1) messeges[id]['seen'] = data[tmp]['seen'];
    } while (id != -1);
  }
}
